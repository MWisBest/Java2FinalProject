...
public class Inventory {
	...
	public void displayKeys() {
		System.out.println(BCDK.messages.getString("display_key") + " " + keys.size());
	}
	...
	public void displayRocks() {
		System.out.println(BCDK.messages.getString("display_rock") + " " + rocks.size());
	}
	...
	public void displayWeapons() {
        System.out.println(BCDK.messages.getString("display_weapon") + " ");
        // only runs if statement if player has at least 1 weapon
        if (!weapons.isEmpty()) {
            weapons.forEach(weapon -> { // 2.1 Lambda  Expression and foreach statement
                System.out.println(weapon.getName() + ": " + weapon.getFullDescription() + BCDK.messages.getString("weapon_info1") + " " + weapon.getDamage()+" "+ BCDK.messages.getString("weapon_info2"));
            });
        } else {
            // player is informed that they do not have any weapons in inventory
            System.out.println(BCDK.messages.getString("weapon_null"));
        }
